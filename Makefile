...
.PHONY: deploy
deploy:
	gzip -c app.min.js | aws s3 --profile=aws-admin cp --content-type 'application/javascript' --content-encoding gzip --cache-control 'public, must-revalidate, proxy-revalidate, max-age=0' - s3://mf-geoadmin4-int-dublin/ol-minimalist/app.min.js
	aws s3 --profile=aws-admin cp  --cache-control 'public, must-revalidate, proxy-revalidate, max-age=0' index.html s3://mf-geoadmin4-int-dublin/ol-minimalist/
